...
| `aaindex_lookup` | phase1 | Implemented. Uses `BIO_MCP_AAINDEX1_PATH` for a local AAindex1 flat-file backend, with packaged fixture fallback. |
| `aaindex_list` | phase1 | Implemented. Lists lightweight AAindex metadata from the configured AAindex1 backend without value tables. |
| `aaindex_sequence_features` | phase1 | Implemented. Uses configured AAindex1 values for per-residue features and summary statistics. |
| `bio_mcp_health` | phase1 | Implemented. Reports package version, Python version, optional dependencies, AAindex backend status, and available tools. |
...
| AAindex2 / AAindex3 parsers | later | Not implemented. Current parser supports AAindex1 only. |
